...
export class ConsumptionRecordService {
  ...
  async findAll(dto: FindConsumptionRecordDto): Promise<any> {
    ...
    const qb = this.consumptionRecordRepository
      .createQueryBuilder('consumptionRecord')
      .leftJoinAndSelect(
        Consumer,
        'consumer',
        'consumer.id = consumptionRecord.consumerId',
      )
      .leftJoinAndSelect(User, 'user', 'user.id = consumptionRecord.createBy')
      .leftJoinAndSelect(
        Balance,
        'balance',
        'balance.orderNum = consumptionRecord.orderNum',
      )
      .select(
        `
    ...
    `,
      ...
      .where('consumptionRecord.isDeleted = :isDeleted', {
      ...
      .andWhere(
        'consumptionRecord.createTime Between :startTime and :endTime',
        { startTime, endTime },
      )
